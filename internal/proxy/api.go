...
func (a *apiHandler) handleSettings(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		pgURL, _ := credential.Get("postgres_url")
		...
		writeJSON(w, 200, map[string]any{
			...
			"database": map[string]any{
				"mode":       dbMode,
				"configured": dbMode == "team" || true, // local is always configured
				"url_masked": maskCredential(pgURL),
			},
	...
	case http.MethodPost:
		...
		// Postgres URL.
		if req.PostgresURL != nil {
			if *req.PostgresURL == "" {
				_ = credential.Delete("postgres_url")
				changes = append(changes, "postgres_url removed (switching to local)")
			} else {
				if err := config.SavePostgresURL(*req.PostgresURL); err != nil {
					writeJSON(w, 500, map[string]string{"error": "failed to save Postgres URL: " + err.Error()})
					return
				}
				changes = append(changes, "postgres_url")
			}
			needRestart = true
		}
